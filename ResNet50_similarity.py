...
def similar():
    ...
    if len(img_data_list)>0:
        ...
        image_similarity_euclidean = calculate_similarity_euclidean(get_feature_vector_fromPIL(img_data_list[0]),
                                                                    get_feature_vector_fromPIL(img_data_list[1]))
        # Caculate cosine similarity: [-1,1], that is, [completedly different, same]
        image_similarity_cosine = calculate_similarity_cosine(get_feature_vector_fromPIL(img_data_list[0]),
                                                              get_feature_vector_fromPIL(img_data_list[1]))

        image_similarity_cosine=image_similarity_cosine[0][0] * 100
        image_similarity_euclidean=image_similarity_euclidean
    else:
        image_similarity_cosine=0
        image_similarity_euclidean=0

    return image_similarity_euclidean,image_similarity_cosine
